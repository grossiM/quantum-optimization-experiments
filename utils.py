...
from qiskit.optimization.converters import IntegerToBinary, InequalityToEquality, LinearEqualityToPenalty
...
## penalty functions. Not yet used in OptScheme

#######################################################################################

def return_higher_penalty(mdl): 
    """
    Return the automatically generated penalty for the given model. 
    :param mdl: the mdl generated by qc_model 
    :return: the automatically generate penalty for quadratic_program_to_qubo plus added terms
    """
    
    qp_ = QuadraticProgram()
    qp_.from_docplex(mdl)
    
    qp_ = IntegerToBinary().convert(qp_)
    qp_ = InequalityToEquality(mode='integer').convert(qp_)
    
    terms = []
    for constraint in qp_.linear_constraints:
        ...
        penalties.extend(abs(coef) for coef in qp_.objective.linear.to_dict().values())
        # add quadratic terms of the object function.
        penalties.extend(abs(coef) for coef in qp_.objective.quadratic.to_dict().values())
        
        # add quadratic norm of sigma and mu
        penalties.extend([math.sqrt(sum([coef ** 2 for coef in qp_.objective.quadratic.to_dict().values()])),
                          math.sqrt(sum([coef ** 2 for coef in qp_.objective.linear.to_dict().values()])),])

    return math.fsum(penalties)


def return_auto_penalty(mdl): 
    """
    Return the automatically generated penalty for the given model. 
    :param mdl: the mdl generated by qc_model 
    :param add_penalty: if True, add the penalty. If false, returns the very same penalty as qiskit code
    :return: the automatically generated penalty for quadratic_program_to_qubo
    """
